...
class AopClient
{
    ...
    /**
     * 生成用于调用收银台SDK的字符串
     * @param string $request SDK接口的请求参数对象
     * @return string
     * @author guofa.tgf
     */
    ...
    /**
     * 页面提交执行方法
     * @param string $request 跳转类接口的request; $httpmethod 提交方式。两个值可选：post、get
    ...
    public function rsaEncrypt($data, $rsaPublicKeyFilePath, $charset)
    {
        ...
        foreach ($blocks as $n => $block) {
            if (!openssl_public_encrypt($block, $chr_text , $res)) {
                echo "<br/>" . openssl_error_string() . "<br/>";
            }
            $encodes[] = $chr_text ;
        }
        $chr_text = implode(",", $encodes);
        return base64_encode($chr_text);
    }
